...
const val TRASFROMATION = "RSA/ECB/PKCS1Padding"
...
object KeyPairHandler {
    ...
    fun getPublicKey(): String {
        ...
        val encodePublicKey = privateKeyEntry?.certificate?.publicKey?.encoded
        return Base64.getEncoder().encodeToString(encodePublicKey)
    }

    fun decryptTheData(encryptedData: String) : String {
        val cipher = Cipher.getInstance(TRASFROMATION)
        val privateKeyEntry = keyStore?.getEntry(KEYSTORE_ALIAS, null) as? KeyStore.PrivateKeyEntry
